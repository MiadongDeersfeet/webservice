...
public class BoardUpdateController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 2) 값 뽑기 전 -- multipart 방식으로 요청이 올 땐 if문을 써서 요청이 잘 왔는가 확인해야합니다.
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			if(multiRequest.getOriginalFileName("reUpfile") != null) { // update_form.jsp 69행 인풋 타입=파일-> 네임속성값
				...
				// INSERT / UPDATE 갈림길에 서버림 --> 해야할 작업이 달라짐
				// INSERT : 이 첨부파일은 어떤 게시글에 달리는가? REF_BNO
				// UPDATE : 원래 있던 파일이 몇 행에 있는 파일이냐??
				
				// 앞에서 원본파일이 있을 땐 fileNo를 넘겼다. -> fileNo 가 몇번인진 몰라도 null 이 아니면 UPDATE 인 것이다. ==> null 이면 INSERT
				...
				if(multiRequest.getParameter("fileNo") != null) {
					...
					// 업데이트 할 땐 기존에 있던 파일을 지워버립시다. -> 파일경로랑 체인지네임이 필요. 앞에서 넘길 것인가 DB에서 긁어올 것인가 ? 우린 앞에서 넘기자! update_form!
					// 기존에 존재하던 첨부파일 삭제
			...
			// board 에는 카테고리, 제목, 내용, 게시글번호가 들어있음 -- at 에는 오리지널네임, 체인지네임, 파일경로, 게시글번호
