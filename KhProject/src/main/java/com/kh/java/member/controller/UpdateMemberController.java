...
public class UpdateMemberController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 2) 요청 시 전달 값 뽑기
		// 뽑기할 때 Key값 알아야해요. userName, email
		...
		// 3) 어디에 담을까? => 오늘은 Map 에 담아볼까요?
		/*
		Map<String, String> map = new HashMap();
		map.put("userName", userName);
		map.put("email", email);
		*/
		// Map.of() :  Key + Value 10개까지 초기화 가능 / 불변맵 반환
		...
		if(result > 0) {
			...
			// 우리에게 생긴 문제점
			// Update에 성공했는데
			// session의 userInfo키 값에는 로그인 당시 / 마이페이지 포워딩 당시 조회했던 값들이 필드에 담겨있기 때문에
			// Update 수행 전 값들이 마이페이지에서 출력이 된다.
