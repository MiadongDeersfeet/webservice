...
<!-- 
	...
	식별자 - 고유한 이름(PK) -> 실제위치가 아니라 논리적인 식별값
...
<body>
	...
	<p>
		Jakarta Server Page Standard Tag Library : JSP에 쓰는 표준 태그 라이브러리 <br>
	...
	2) Standard-1.2.5.jar파일 4개 모두 다운받기 <br>
	3) WEB-INF/lib폴더에 추가해주기 <br>
	
	<h4>* JSTL 선언</h4>
	
	<p>
		JSTL을 사용하고자 하는 해당 JSP파일 상단에 <br>
		(보통 page 지시어 바로 밑에) <br>
	...
	${ outStr } <br>
	<c:out value="${ outStr }" />
	<!--  둘 다 똑같이 나오지만 태그가 아니라 진짜 문자열로 쓰고싶을 수 있잖아요. 아래와 같은 식으로 -->
	&lt;strong>강한정보&lt;/strong>
	<!--  escapeXml은 적어만 놨어요. 위와 같이 쓰는 게 훨씬 편해요~! -->
	...
	<pre>
		...
		[ 표현법 ]
		&lt;c:if test="조건식">
			조건식이 true일 경우 실행할 내용 -- 보통 우리는 출력할 내용이겠죠?
		&lt;/c:if>	
		
		- Java의 단일 if문과 비슷한 역할을 수행하는 태그
		- 조건식은 test라는 속성에 작성
		※(조건식을 만들 때는 반드시 EL구문으로 작성해야함!!!!!!!!!!) => 어제 배운 연산자들 말하는 겁니다. 자바처럼 하면 안돼요^^;
	</pre>
	<%-- 
	...
	<pre>
		...
		choose는 when과 otherwise만 가질 수 있어요.
		when과 otherwise는 choose의 자식요소들이에요.
	</pre>
	<%--  
	<% if(조건1) { %>
	...
	회원 등급 출력 : <!--  choose 안에는 when과 otherwise만 들어갈 수 있으니까 태그 사이에는 '주석도' 달지마세요!!!!! -->
	<c:choose>
	...
	<pre>
		[ 표현법 ]
		for loop문
		&lt;c:forEach var="속성명(제어변수)" begin="초기값" end="끝값" step="몇씩증가">
			반복시킬 내용
		&lt;/c:forEach>
		
		step은 생략 시 기본값이 1 => 보통 잘 안적어요.
		
		향상된 for문
		&lt;c:forEach var="속성명(제어변수)" items="순차적으로 접근할 배열/컬렉션" varStatus="상태값">
			반복시킬 내용
		&lt;/c:forEach>
	...
	<table border="1">
		...
		<tbody> <!-- 237행 : var="p" 하나가 Person 객체임 --> <!-- 275행 : 스클립틀릿이 아닌 EL구문에서는 get 안쓰고 필드명을 바로 써준다. -->
			<c:choose>
				...
				<c:otherwise>
					<c:forEach var="p" items="${ requestScope.list }" varStatus="s">
						<tr>
							<!--  index : 0부터 / count : 1부터 -->
							<td>${ s.count }</td>
							<td>${ p.name }</td>
	...
	<pre>
		...
		&lt;c:forTokens var="값을 보관할 속성명" items="분리하고자 하는 문자열" delims="구분자">
			...
			반복적으로 실행할 문구(여기 안에는 웬만하면 출력만 하세요)
