...
public class ElServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// JSP로 포워딩할 때 request에 담아서 보내줬음! --> Servlet 내장객체 / Scope 객체
		
		// application / session / request / page => ※ 실질적으로 많이 사용하는 것은 request scope와 session scope / ※ SqlSession과는 연관이 없습니다.
		
		/* <= 여기가 제일 크고
		 * 
		 * 1. application scope : ServletContext
		 * 한 애플리케이션 당, 단 한 개 존재하는 객체입니다.
		 * 이 영역에 데이터를 담으면 애플리케이션 전역에서 사용 가능
		 * 
		 * 2. session scope : HttpSession
		 * 하나의 브라우저 당, 한 개 존재하는 객체
		 * 로그인 정보가 세션 스코프에 담겨있기 때문에 로그인 후 새로운 브라우저를 열어도 로그인이 유지된 상태임.
		 ...
		 * 서블릿이 클라이언트한테 응답을 해주고 나면 request와 response는 소멸이 됩니다.
		 * 이 영역에 데이터를 담으면 해당 request객체를 포워딩 받는 응답 JSP에서만 사용가능(일회성)
		 * 
		 ...
		 * 							   .
		 * 
		 * <= 여기가 제일 작아요 // 범위에 대한 ':' 뒤에는 자바 타입입니다.
		 */ 
		...
		session.setAttribute("lecture", new Person("김윤기", 30, "강북"));
